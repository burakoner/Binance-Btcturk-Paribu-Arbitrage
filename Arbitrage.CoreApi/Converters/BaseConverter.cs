...
namespace Arbitrage.CoreApi.Converters
{
    ...
    public abstract class BaseConverter<T> : JsonConverter where T : struct
    {
        ...
        {
            string stringValue = GetValue((T)value);
            if (quotes)
            {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            if (!GetValue(reader.Value.ToString(), out T result))
            {
        ...
        {
            KeyValuePair<T, string> mapping = Mapping.FirstOrDefault(kv => kv.Value.Equals(value, StringComparison.InvariantCultureIgnoreCase));
